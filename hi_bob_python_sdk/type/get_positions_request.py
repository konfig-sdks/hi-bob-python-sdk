# coding: utf-8

"""
    Bob API

    Access your employees data with the Bob API

    The version of the OpenAPI document: 1.0.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from hi_bob_python_sdk.type.get_positions_request_fields import GetPositionsRequestFields

class RequiredGetPositionsRequest(TypedDict):
    fields: GetPositionsRequestFields

    filters: typing.List[FilterInstruction]

class OptionalGetPositionsRequest(TypedDict, total=False):
    # Whether to include the additional \"humanReadable\" entry in the response.
    includeHumanReadable: bool

class GetPositionsRequest(RequiredGetPositionsRequest, OptionalGetPositionsRequest):
    pass
