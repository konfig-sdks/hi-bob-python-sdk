# coding: utf-8

"""
    Bob API

    Access your employees data with the Bob API

    The version of the OpenAPI document: 1.0.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING


class RequiredCreateFieldRequest(TypedDict):
    # The name of the field.
    name: str

    # The category of the field.
    category: str

    # The type of field. Supported field types: text, text-area, number, date, list, multi-list, hierarchy-list, currency, employee-reference, document.
    type: str

class OptionalCreateFieldRequest(TypedDict, total=False):
    # The description of the field.
    description: str

    # When true, this field keeps the history of its values, each being active starting from a certain date. The default value is false.
    historical: str

class CreateFieldRequest(RequiredCreateFieldRequest, OptionalCreateFieldRequest):
    pass
